...
Note that although `assertEqual()` assertion statement allows for an optional custom message as an optional third parameter, the `assertTrue()` assertion statement requires and allows only two parameters. Try adding your own custom message as a Python text string for as a third parameter in the `assertTrue()` assertion statement, and you will see that a `TypeError` will be raised.
